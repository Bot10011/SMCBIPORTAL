...
const DashboardOverview = () => {
  ...
  // Lightweight cache for faster perceived loads
  const CLASSROOM_CACHE_KEY = useMemo(() => `google_classroom_cache_${user?.id || 'anon'}`,[user?.id]);
  const CLASSROOM_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

  // Cache key for avatar to reduce flashing when reloading
  const AVATAR_CACHE_KEY = useMemo(() => `user_avatar_url_${user?.id || 'anon'}`,[user?.id]);

  // Try to restore cached avatar immediately to avoid initials flash
  useEffect(() => {
    try {
  ...
  const fetchGoogleClassroomData = useCallback(async () => {
    try {
      // Keep existing data during refresh for better UX
      setGoogleClassroomData(prev => ({ ...prev, isLoading: prev.courses.length === 0 }));
      ...
      const coursesResponse = await fetch('https://classroom.googleapis.com/v1/courses?courseStates=ACTIVE&pageSize=50', {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (coursesResponse.ok) {
        const coursesData = await coursesResponse.json();
        const courses = coursesData.courses || [];
        // Immediate update with courses
        setGoogleClassroomData(prev => ({ ...prev, courses }));
        ...
        const courseWorkArrays = await Promise.allSettled(courseWorkPromises).then(results => results.flatMap(r => r.status === 'fulfilled' ? r.value : []));
        ...
        const submissionPromises = courseWork.map(async (work: { id: string; courseId: string; title: string; description: string; materials: Array<{ title: string; driveFile?: { driveFile: { id: string } } }>; state: string; creationTime: string; dueDate?: { year: number; month: number; day: number }; dueTime?: { hours: number; minutes: number }; maxPoints?: number }) => {
          try {
            ...
            if (submissionResponse.ok) {
              ...
              const subs = submissionData.studentSubmissions || [];
              if (subs.length) {
                setGoogleClassroomData(prev => ({ ...prev, submissions: [...prev.submissions, ...subs] }));
              }
              return subs;
        ...
        const submissionArrays = await Promise.allSettled(submissionPromises).then(results => results.flatMap(r => r.status === 'fulfilled' ? r.value : []));
        const submissions = submissionArrays.flat();
        const finalData = { courses, courseWork, submissions };
        setGoogleClassroomData({ ...finalData, isLoading: false });
        try {
          localStorage.setItem(CLASSROOM_CACHE_KEY, JSON.stringify({ ts: Date.now(), data: finalData }));
        } catch { /* ignore */ }
  ...
  }, [user?.id, fetchGoogleClassroomData, CLASSROOM_CACHE_KEY, CLASSROOM_CACHE_TTL_MS]);
    ...
    const fetchStudentProfile = async () => {
      if (user?.id) {
        try {
          ...
          // Preload image before swapping to avoid visible flash
          if (pictureUrl) {
            ...
            try { localStorage.setItem(AVATAR_CACHE_KEY, pictureUrl); } catch { /* ignore */ }
          } else {
            setProfilePictureUrl(null);
  ...
  }, [user?.id, user?.email, AVATAR_CACHE_KEY]);
              ...
              <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-8">
                ...
                <div className="relative w-32 h-32 sm:w-40 sm:h-40 rounded-full flex items-center justify-center overflow-hidden ring-4 ring-white shadow-[0_10px_25px_-10px_rgba(0,0,0,0.5)]">
                  {profilePictureUrl ? (
                    ...
                    <div className="absolute inset-0 w-full h-full rounded-full bg-blue-600 flex items-center justify-center">
                      <span className="text-white font-extrabold text-5xl sm:text-6xl tracking-tight">
                        {(() => {
                          ...
                          const initial = emailLocal?.[0] ? emailLocal[0].toUpperCase() : 'U';
                          return initial;
                        })()}
                      </span>
                    </div>
