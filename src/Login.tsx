...
const Login: React.FC<LoginProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        ...
        // If user is using default password, force them to change it
        if (isDefaultPassword) {
          setForcePasswordChangeEmail(fullEmail);
        ...
        if (authError instanceof AuthError && authError.message === 'Email not confirmed') {
          // If user is using default password, force them to change it
          if (isDefaultPassword) {
            setForcePasswordChangeEmail(fullEmail);
            setShowForcePasswordChange(true);
  ...
  // Handle forced password change success
  const handleForcePasswordChangeSuccess = () => {
    setShowForcePasswordChange(false);
    setForcePasswordChangeEmail('');
    // Clear the form
    setFormData({ username: '', password: '' });
    toast.success('Password changed successfully! Please login with your new password.');
  };

  // Handle forced password change cancel
  const handleForcePasswordChangeCancel = () => {
    setShowForcePasswordChange(false);
    setForcePasswordChangeEmail('');
    // Clear the form
    setFormData({ username: '', password: '' });
  ...
  return (
    <div className="relative">
      ...
      <div
        className={`transition-all duration-700 ease-out ${showInstructionModal || showForgotModal || showForcePasswordChange ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}
        style={{
          animation: !showInstructionModal && !showForgotModal && !showForcePasswordChange ? 'fadeInUp 0.8s ease-out' : 'none'
        }}
      >
...
To request an account, please approach your adviser or visit the Program Head's office. Only authorized users can be registered in the portal. If you believe you should have access, provide your full name, student ID, and a valid email address.            </p>
      ...
      {showForcePasswordChange && (
        <ForcePasswordChange
          email={forcePasswordChangeEmail}
          onSuccess={handleForcePasswordChangeSuccess}
          onCancel={handleForcePasswordChangeCancel}
        />
      )}
    </div>
  );
