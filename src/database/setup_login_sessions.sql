-- Setup script for login_sessions table
...
CREATE TABLE IF NOT EXISTS public.login_sessions (
    ...
    location_source VARCHAR(10) CHECK (location_source IN ('GPS', 'IP', 'UNKNOWN')), -- Source of location data
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'login_sessions' AND column_name = 'location_accuracy') THEN
        ALTER TABLE public.login_sessions ADD COLUMN location_accuracy DECIMAL(8, 2);
    END IF;
    
    -- Add location_source column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'login_sessions' AND column_name = 'location_source') THEN
        ALTER TABLE public.login_sessions ADD COLUMN location_source VARCHAR(10) CHECK (location_source IN ('GPS', 'IP', 'UNKNOWN'));
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_login_sessions_location_source ON public.login_sessions(location_source);
...
-- Success message
SELECT 'login_sessions table and related objects created successfully!' as status; 